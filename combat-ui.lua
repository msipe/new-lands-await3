...
local createDieFromConstruct = ____dice_2Dfactory.createDieFromConstruct
local ____dice_2Dconstructs = require("game.dice-constructs.index")
local getDieConstructById = ____dice_2Dconstructs.getDieConstructById
function isDieSettled(self, die)
    local speed = math.sqrt(die.vx * die.vx + die.vy * die.vy)
    return speed <= 60 and math.abs(die.spin) <= 1.3
end
function getDieEnergyCost(self, state, dieId)
...
function getProjectedPendingEnergySpend(self, uiState, state)
    local pendingSpend = 0
    for ____, dieId in ipairs(uiState.rolledPlayerDieIds) do
        do
            if __TS__ArrayIncludes(state.rolledPlayerDieIds, dieId) then
                goto __continue183
            end
            pendingSpend = pendingSpend + getDieEnergyCost(nil, state, dieId)
        end
        ::__continue183::
    end
    return pendingSpend
end
function getProjectedPlayerEnergy(self, uiState, state)
    return math.max(
        0,
        state.playerEnergyCurrent - getProjectedPendingEnergySpend(nil, uiState, state)
    )
end
function canAffordAnyUnthrownDie(self, uiState, state)
    local projectedEnergy = getProjectedPlayerEnergy(nil, uiState, state)
    for ____, die in ipairs(state.player.dice) do
        do
            local dieId = die.id
            ...
            if die.energyCost <= projectedEnergy then
                return true
            end
        end
        ::__continue188::
...
function ____exports.canPlayerThrow(self, uiState, state)
    return state.phase == "player-turn" and uiState.pendingRound == nil and canAffordAnyUnthrownDie(nil, uiState, state)
end
...
function buildSortedInspectorSidesFromRaw(self, rawSides)
    local sorted = __TS__ArraySort(
        ...
        function(____, left, right)
            if right.power ~= left.power then
                return right.power - left.power
            end
            return left.baseIndex - right.baseIndex
        end
    )
    local ____opt_35 = sorted[1]
    local highestPower = ____opt_35 and ____opt_35.power or 0
    local ____opt_37 = sorted[#sorted]
    local lowestPower = ____opt_37 and ____opt_37.power or 0
...
local RED = {r = 0.94, g = 0.36, b = 0.36}
local ENERGY_ORB_ACTIVE = {r = 0.98, g = 0.62, b = 0.16}
local ENERGY_ORB_INACTIVE = {r = 0.31, g = 0.21, b = 0.14}
    ...
    uiState.playerDice = __TS__ArrayMap(
        ...
        function(____, die, index)
            ...
            return {
                ...
                owner = "player",
                isItemDie = __TS__StringStartsWith(die.id, "equipped-"),
                tint = die.tint,
                combatDieId = die.id,
                label = die.name,
                displayLabel = nil,
...
function ____exports.syncSpawnedPlayerDice(self, uiState, state)
    local knownCombatIds = __TS__New(
        Set,
        __TS__ArrayMap(
            uiState.playerDice,
            function(____, vd) return vd.combatDieId end
        )
    )
    local newSpawned = __TS__ArrayFilter(
        state.player.dice,
        function(____, cd) return not knownCombatIds:has(cd.id) end
    )
    if #newSpawned == 0 then
        return
    end
    local totalCount = #state.player.dice
    local poolSlotLayout = computePoolSlotLayout(nil, uiState.layout, totalCount)
    for ____, vd in ipairs(uiState.playerDice) do
        do
            local combatIndex = __TS__ArrayFindIndex(
                state.player.dice,
                function(____, cd) return cd.id == vd.combatDieId end
            )
            if combatIndex == -1 then
                goto __continue20
            end
            local position = makePoolPosition(nil, uiState.layout, combatIndex, totalCount)
            vd.parkX = position.x
            vd.parkY = position.y
            vd.size = poolSlotLayout.size
            if vd.state == "parked" then
                vd.x = position.x
                vd.y = position.y
            end
        end
    ...
    for ____, die in ipairs(newSpawned) do
        local index = __TS__ArrayIndexOf(state.player.dice, die)
        local position = makePoolPosition(nil, uiState.layout, index, totalCount)
        local ____uiState_playerDice_1 = uiState.playerDice
        ____uiState_playerDice_1[#____uiState_playerDice_1 + 1] = {
            id = "player-pool-" .. die.id,
            owner = "player",
            isSpawnedDie = true,
            tint = die.tint,
...
local function updateRollingFaceLabels(self, uiState, state, dt)
    local ____array_3 = __TS__SparseArrayNew(unpack(uiState.enemyArenaDice))
    __TS__SparseArrayPush(
        ____array_3,
        unpack(uiState.arenaPlayerDice)
    )
    local activeDice = {__TS__SparseArraySpread(____array_3)}
...
local function updateEnemyParkingTransitions(self, uiState, dt)
    for ____, die in ipairs(uiState.enemyParkedDice) do
        do
            ...
            local startSize = die.parkStartSize or die.size
            local targetSize = die.parkSize or die.size
            ...
            if progress >= 1 then
                ...
                die.parkStartX = nil
                die.parkStartY = nil
                die.parkStartSize = nil
                die.parkProgress = nil
...
local function updateDieFlashes(self, uiState, dt)
    local ____array_4 = __TS__SparseArrayNew(unpack(uiState.playerDice))
    __TS__SparseArrayPush(
        ____array_4,
    ...
    local allDice = {__TS__SparseArraySpread(____array_4)}
...
local function parkPlayerArenaDice(self, uiState, state)
    local consumed = __TS__New(Set)
    for ____, die in ipairs(uiState.arenaPlayerDice) do
        do
            local stillExists = die.combatDieId == nil or __TS__ArraySome(
                state.player.dice,
                function(____, cd) return cd.id == die.combatDieId end
            )
    ...
    if consumed.size > 0 then
        uiState.playerDice = __TS__ArrayFilter(
            uiState.playerDice,
            function(____, vd) return not consumed:has(vd.id) end
        )
    end
...
local function finalizeRoundTransition(self, uiState, state)
    ...
    uiState.playerDice = __TS__ArrayFilter(
        uiState.playerDice,
        function(____, vd) return __TS__ArraySome(
            state.player.dice,
            function(____, cd) return cd.id == vd.combatDieId end
        ) end
    )
...
local function enqueueSettledPlayerDice(self, uiState)
    ...
    for ____, dieId in ipairs(uiState.pendingPlayerDieIds) do
        do
            ...
            visualDie.flashColor = nil
            visualDie.displayLabel = nil
...
local function canAdvancePlayerTurn(self, uiState, state)
    if state.phase ~= "player-turn" or uiState.pendingRound ~= nil then
        return false
    end
    if #uiState.pendingPlayerDieIds > 0 or #uiState.readyPlayerDieIds > 0 then
        return false
    end
    return not canAffordAnyUnthrownDie(nil, uiState, state)
end
local function updateEnemyPresentation(self, uiState, state, dt)
    ...
    if #uiState.enemyArenaDice == 0 and #uiState.enemyPendingDice > 0 then
        ...
        if immediate then
            local ____uiState_enemyArenaDice_16 = uiState.enemyArenaDice
            ____uiState_enemyArenaDice_16[#____uiState_enemyArenaDice_16 + 1] = immediate
        end
    end
    while #uiState.enemyPendingDice > 0 and uiState.enemySpawnTimer >= uiState.enemySpawnInterval do
        ...
        if next then
            ...
            ____uiState_enemyArenaDice_17[#____uiState_enemyArenaDice_17 + 1] = next
        end
    end
    settleEnemyArenaDice(nil, uiState)
    if #uiState.enemyPendingDice == 0 then
        if areDiceSettled(nil, uiState.enemyArenaDice) then
            uiState.enemySettleTimer = uiState.enemySettleTimer + dt
            if uiState.enemySettleTimer >= uiState.enemySettleDelay then
...
local function executePlayerThrow(self, uiState, state, dragged, startX, startY, x, y)
    if not dragged.combatDieId then
        return false
    end
    if __TS__ArrayIncludes(uiState.rolledPlayerDieIds, dragged.combatDieId) or not ____exports.canPlayerThrow(nil, uiState, state) or getDieEnergyCost(nil, state, dragged.combatDieId) > getProjectedPlayerEnergy(nil, uiState, state) or not isInsideArena(nil, uiState, x, y) then
    ...
    if not __TS__ArrayFind(
        ...
        local ____uiState_arenaPlayerDice_18 = uiState.arenaPlayerDice
        ____uiState_arenaPlayerDice_18[#____uiState_arenaPlayerDice_18 + 1] = dragged
    end
    local ____uiState_rolledPlayerDieIds_19 = uiState.rolledPlayerDieIds
    ____uiState_rolledPlayerDieIds_19[#____uiState_rolledPlayerDieIds_19 + 1] = dragged.combatDieId
    if not __TS__ArrayIncludes(uiState.pendingPlayerDieIds, dragged.combatDieId) then
        local ____uiState_pendingPlayerDieIds_20 = uiState.pendingPlayerDieIds
        ____uiState_pendingPlayerDieIds_20[#____uiState_pendingPlayerDieIds_20 + 1] = dragged.combatDieId
    end
    return true
end
local function parkPlayerDie(self, die)
    die.state = "parked"
    if die.parkX ~= nil and die.parkY ~= nil then
        die.x = die.parkX
        die.y = die.parkY
...
local function rejectPlayerThrowDrop(self, uiState, die, dropX, dropY, text)
    parkPlayerDie(nil, die)
    die.flashTimer = RESOLVE_FLASH_DURATION
    die.flashColor = RED
    local ____uiState_floatingPopups_21 = uiState.floatingPopups
    ____uiState_floatingPopups_21[#____uiState_floatingPopups_21 + 1] = {
        x = dropX,
...
function ____exports.fastForwardCombatUi(self, uiState, state)
    ...
    if state.phase == "player-turn" then
        ...
        if canAdvancePlayerTurn(nil, uiState, state) then
...
function ____exports.updateCombatUiState(self, uiState, state, dt)
    ...
    if uiState.queuedPlayerThrow and uiState.pendingRound == state.round then
        ____exports.fastForwardCombatUi(nil, uiState, state)
    end
    if uiState.queuedPlayerThrow and state.phase == "player-turn" and uiState.pendingRound == nil then
        local queued = uiState.queuedPlayerThrow
        local queuedDie = __TS__ArrayFind(
            uiState.playerDice,
            function(____, die) return die.id == queued.dieId end
        )
        if queuedDie then
            executePlayerThrow(
                nil,
        ...
        uiState.queuedPlayerThrow = nil
    end
    updateArenaDice(nil, uiState, dt)
    updateEnemyParkingTransitions(nil, uiState, dt)
...
function ____exports.drainSettledPlayerDieIds(self, uiState)
    ...
    uiState.settledPlayerDieIds = {}
    uiState.readyPlayerDieIds = __TS__ArrayFilter(
        uiState.readyPlayerDieIds,
        function(____, dieId) return not __TS__ArrayIncludes(settled, dieId) end
    )
    return settled
end
...
function ____exports.consumeRequestedPlayerTurnEnd(self, uiState)
    if not uiState.requestedPlayerTurnEnd then
        return false
    end
...
local function getTurnButtonRect(self, layout)
    local width = 148
    local height = 36
    return {x = layout.poolX + layout.poolWidth - width - 14, y = layout.poolY - height - 12, width = width, height = height}
end
...
local function findVisualDieByCombatId(self, uiState, dieId)
    local ____array_22 = __TS__SparseArrayNew(unpack(uiState.arenaPlayerDice))
    __TS__SparseArrayPush(
        ____array_22,
        unpack(uiState.playerDice)
    )
    __TS__SparseArrayPush(
        ____array_22,
        unpack(uiState.enemyArenaDice)
...
local function findVisualDieById(self, uiState, dieId)
    local ____array_23 = __TS__SparseArrayNew(unpack(uiState.playerDice))
    __TS__SparseArrayPush(
        ____array_23,
        unpack(uiState.arenaPlayerDice)
    )
    ...
    return __TS__ArrayFind(
        ...
        function(____, die) return die.id == dieId end
    )
end
    ...
    for ____, popup in ipairs(popups) do
        do
            if popup.spawnedDie then
                ...
                local spawnedInspectorSides = buildSortedInspectorSidesFromRaw(
                    nil,
                    __TS__ArrayMap(
                        ...
                        function(____, side, index) return {
                            ...
                            power = type(side.power) == "number" and __TS__NumberIsFinite(side.power) and roundPower(nil, side.power) or 0,
                            baseIndex = index
                        } end
                    )
                )
                local spawnedInspectorTotalPower = roundPower(
                    nil,
                    __TS__ArrayReduce(
                ...
                local ____uiState_arenaPlayerDice_32 = uiState.arenaPlayerDice
                ____uiState_arenaPlayerDice_32[#____uiState_arenaPlayerDice_32 + 1] = {
                    ...
                    spawnedInspectorSides = spawnedInspectorSides,
                    spawnedInspectorEnergyCost = spawnedInspectDie.energyCost,
                    spawnedInspectorTotalPower = spawnedInspectorTotalPower
                }
...
local function buildInspectorViewFromCombatDie(self, state, dieId, owner)
    local dice = owner == "player" and state.player.dice or state.enemy.dice
    local die = __TS__ArrayFind(
        dice,
        function(____, entry) return entry.id == dieId end
    )
    ...
    local snapshot = getDiePowerSnapshot(nil, state, die.id)
    if snapshot and #snapshot.orderedFaces > 0 then
        local orderedSides = {}
        for ____, face in ipairs(snapshot.orderedFaces) do
            do
                ...
                orderedSides[#orderedSides + 1] = {
                    id = side.id,
                    label = side.label,
                    description = describeDieSide(nil, side),
                    power = face.power,
    ...
    local fallbackSides = buildSortedInspectorSidesFromRaw(
        nil,
        __TS__ArrayMap(
            die.sides,
            function(____, side, index) return {
                ...
                label = side.label,
                description = describeDieSide(nil, side),
                power = type(side.power) == "number" and roundPower(nil, side.power) or 0,
                baseIndex = index
            } end
        )
    )
...
local function getInspectorDieView(self, uiState, state)
    local inspector = uiState.inspector
    ...
    return buildInspectorViewFromCombatDie(nil, state, inspector.combatDieId, inspector.owner)
end
local function getInspectorSideIndexAt(self, uiState, state, x, y)
    local inspectorDie = getInspectorDieView(nil, uiState, state)
    if not inspectorDie then
        return nil
    end
    local layout = getInspectorLayout(nil, #inspectorDie.sides)
...
local function getInspectorEnergyCost(self, uiState, state)
    local inspector = uiState.inspector
    if not inspector then
        return nil
    end
    ...
    local dice = inspector.owner == "player" and state.player.dice or state.enemy.dice
    local die = __TS__ArrayFind(
        dice,
        function(____, entry) return entry.id == inspector.combatDieId end
    )
    return die and math.max(0, die.energyCost) or nil
end
local function drawDieInspector(self, uiState, state)
    ...
    local ____opt_43 = uiState.inspector
    local ownerLabel = (____opt_43 and ____opt_43.owner) == "player" and "Ally" or "Enemy"
    ...
    love.graphics.print(
        tostring(#inspectorDie.sides) .. " faces",
        panel.x + 20,
        panel.y + 40
    )
    ...
    local ____opt_45 = uiState.inspector
    local ____temp_49 = ____opt_45 and ____opt_45.hoveredSideIndex
    if ____temp_49 == nil then
        local ____opt_47 = uiState.inspector
        ____temp_49 = ____opt_47 and ____opt_47.selectedSideIndex
    end
    local activeSideIndex = ____temp_49
    for ____, tile in ipairs(layout.tiles) do
    ...
    if activeSideIndex == nil then
        ...
        local activeSide = inspectorDie.sides[activeSideIndex + 1]
        local criticalTags = __TS__ArrayFilter(
            {activeSide.isCriticalHit and "CRITICAL HIT" or nil, activeSide.isCriticalMiss and "CRITICAL MISS" or nil},
            function(____, tag) return tag ~= nil end
        )
        local criticalSuffix = #criticalTags > 0 and (" [" .. table.concat(criticalTags, " | ")) .. "]" or ""
        local detailsLine = (((((((("Face " .. tostring(activeSideIndex + 1)) .. ": ") .. activeSide.label) .. " (Power ") .. tostring(activeSide.power)) .. ")") .. criticalSuffix) .. " - ") .. activeSide.description
        love.graphics.printf(
            detailsLine,
            detailsRect.x + 14,
            detailsRect.y + 20,
...
function ____exports.onCombatMousePressed(self, uiState, state, x, y, button)
    ...
    local turnButton = getTurnButtonState(nil, uiState, state)
    ...
    if uiState.inspector then
        ...
        local inspectorLayout = getInspectorLayout(nil, #inspectorDie.sides)
    ...
    for ____, die in ipairs(uiState.playerDice) do
        do
            ...
            uiState.drag = {
                ...
                lastY = y,
                lastDt = 1 / 60,
                skipOnRelease = state.phase ~= "player-turn" or uiState.pendingRound ~= nil
            }
            return
        end
        ::__continue297::
...
function ____exports.onCombatMouseMoved(self, uiState, state, x, y, dx, dy)
    ...
    local dragged = __TS__ArrayFind(
        uiState.playerDice,
        function(____, die)
            local ____die_id_52 = die.id
            local ____opt_50 = uiState.drag
            return ____die_id_52 == (____opt_50 and ____opt_50.dieId)
        end
    )
    if not dragged then
        return
...
function ____exports.onCombatMouseReleased(self, uiState, state, x, y, button)
    ...
    if isInsideArena(nil, uiState, x, y) and getDieEnergyCost(nil, state, dragged.combatDieId) > getProjectedPlayerEnergy(nil, uiState, state) then
        rejectPlayerThrowDrop(
            nil,
            uiState,
            dragged,
            x,
...
local function drawDie(self, die)
    ...
    if die.flashTimer ~= nil and die.flashTimer > 0 then
        ...
        local flashColor = die.flashColor or WHITE
...
local function drawFloatingResolutionPopups(self, uiState)
    for ____, popup in ipairs(uiState.floatingPopups) do
        ...
        if popup.powerTone == "positive" then
            love.graphics.setColor(0.72, 0.96, 0.78, alpha)
        elseif popup.powerTone == "negative" then
            love.graphics.setColor(0.96, 0.56, 0.56, alpha)
        elseif popup.source == "player" then
            love.graphics.setColor(0.72, 0.96, 0.78, alpha)
        else
...
local function drawEnergyOrbs(self, currentEnergy, maxEnergy, x, y)
    local count = math.max(
        0,
        math.floor(maxEnergy)
    )
    local activeCount = math.max(
        0,
        math.min(
            count,
    ...
    do
        ...
        while index < count do
            ...
            local fill = active and ENERGY_ORB_ACTIVE or ENERGY_ORB_INACTIVE
            love.graphics.setColor(fill.r, fill.g, fill.b, active and 0.96 or 0.84)
            ...
            love.graphics.setColor(1, 0.9, 0.7, active and 0.52 or 0.28)
            love.graphics.circle("line", cursorX, y, radius)
            cursorX = cursorX + (radius * 2 + gap)
            index = index + 1
        end
    end
end
function ____exports.drawCombatUi(self, uiState, state)
    ...
    local turnButton = getTurnButtonState(nil, uiState, state)
    if turnButton.visible then
        local buttonRect = getTurnButtonRect(nil, layout)
        if turnButton.enabled then
        ...
        love.graphics.setColor(1, 1, 1, turnButton.enabled and 0.98 or 0.84)
        love.graphics.printf(
            turnButton.label,
            buttonRect.x,
            buttonRect.y + 11,
    ...
    local projectedEnergy = math.max(
        0,
        math.floor(getProjectedPlayerEnergy(nil, uiState, state))
    )
    local visibleEnergy = state.phase == "player-turn" and projectedEnergy or state.playerEnergyCurrent
    love.graphics.setColor(WHITE.r, WHITE.g, WHITE.b)
    love.graphics.print(
